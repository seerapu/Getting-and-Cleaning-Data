...
# set working directory to the folder when the zip file is unizipped
# unzip creates UCI HAR Dataset directory

# Installs the required package data.table if missing
if (!require("data.table")) {
  install.packages("data.table")
}

# Installs the required package reshape2 if missing
if (!require("reshape2")) {
...
# Load: activity labels from the directory UCI HAR Dataset extracted from the zip file
actlabels <- read.table("./UCI HAR Dataset/activity_labels.txt")[,2]

# Load: features from the directory UCI HAR Dataset extracted from the zip file
...
write.table(tidy_data, file = "./cleanData.txt")
